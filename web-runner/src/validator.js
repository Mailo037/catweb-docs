...
import { parseCatWebJson } from './parser.js';
...
export const UDIM2_REGEX = /^\{\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*\}\s*,\s*\{\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*\}$/;
export const UDIM_REGEX = /^(-?[\d.]+)\s*,\s*(-?[\d.]+)$/;
export const HEX_COLOR_REGEX = /^#[0-9a-fA-F]{3,8}$/;
// CatWeb generates 2-3 printable ASCII characters for global IDs
export const GLOBAL_ID_REGEX = /^[\x20-\x7E]{2,3}$/;
 ...
 */
export const validateInput = validateCatWeb;
